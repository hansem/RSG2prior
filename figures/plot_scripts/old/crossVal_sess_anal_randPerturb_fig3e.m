function crossVal_sess_anal_randPerturb_fig3e

% 2019/4/9
% debug: flip x & y back in surf & N'

% 2019/3/18
% speed vs Xu, tp vs speed (colormaps)
% average across all data sets (sessions with # neurons>50, conditions)
% H: 161206 161211 161218 161221 161222 161223
% G: 170506 170507 170511 170818 170821 170822 170823

% check p(prior-dependent & ts-dependent neurons)
...
anNm={'H'}; % {'G'}; % {'H'}; % 'G'; % {'H';'G'}; %'G'; % 'H'; % {'H';'G'}; % both animal
sessId={'161218'}; % {'170511'}; % {'161218'}; %{{'161218'};... % {{'161206';'161211';'161218';'161221';'161222';'161223'};... % H: 6 sessions
%     {'170506';'170507';'170511';'170818';'170821';'170822';'170823'}}; % G: 7 sessions

% % specifying session
% anNm='G'; % 'H'; % 'G'; %  H: 161218 161222 161211
% sessId='170511'; % '161218'; % '170506'; % G: 170817 170506
...
if numel(sessId)>1 % pooling across sessions
    if numel(anNm)>1        % across animals
        idPr=[];
        tp=[];
        ts=[];
        idHE=[];
        ...
        for iAnimal=1:nAnimal
            for iSess=1:length(sessId{iAnimal})
                d1=load(['trajKS_' sessId{iAnimal}{iSess} '_pr_' anNm{iAnimal} '_condSpec_GPFA_CV_cellFullOnly.mat']);
                
                sId=[sId; repmat(str2num(sessId{iAnimal}{iSess}),length(d1.ts),1)];
                idPr=[idPr; d1.idPr]; % 1 short 2 long
                tp=[tp; d1.tp];
                ts=[ts; d1.ts];
                idHE=[idHE; 2-d1.idHandEye]; % 0 hand 1 eye
                theta=[theta; d1.theta-1]; % d1.theta==180; % 0 right 1 left
                sSet=[sSet;d1.sSet];
                
                d2=load(['trajKS_' sessId{iAnimal}{iSess} '_tp_' anNm{iAnimal} '_condSpec_GPFA_CV_cellFullOnly.mat']);
                % speed estimation: measure instantanious distance (c.f. KiNeT)
                nTrial=length(d2.D);
                vTmp=nan(nTrial,1);
                for iT=1:nTrial
                    vTmp(iT)=mean(sqrt(sum(diff(d2.D(iT).data,1,2).^2,1)));
                end % iT=1:nTrial
                v=[v;vTmp];
            end % sess
        end % animal
    else % one animal
        tmpSessId=sessId{strcmp(anNm,animalNm)}; % 161218
        idPr=[];
        tp=[];
        ts=[];
        idHE=[];
        ...
        v=[];
        sId=[];
        for iSess=1:length(tmpSessId)
            d1=load(['trajKS_' tmpSessId{iSess} '_pr_' anNm '_condSpec_GPFA_CV_cellFullOnly.mat']);
            
            sId=[sId; repmat(str2num(tmpSessId{iSess}),length(d1.ts),1)];
            idPr=[idPr; d1.idPr]; % 1 short 2 long
            tp=[tp; d1.tp];
            ts=[ts; d1.ts];
            idHE=[idHE; 2-d1.idHandEye]; % 0 hand 1 eye
            ...
            d2=load(['trajKS_' tmpSessId{iSess} '_tp_' anNm '_condSpec_GPFA_CV_cellFullOnly.mat']);
            % speed estimation: measure instantanious distance (c.f. KiNeT)
            nTrial=length(d2.D);
            vTmp=nan(nTrial,1);
            for iT=1:nTrial
    ...
    tsId=nan(size(ts)); for iPr=1:nPr,for j=1:nTspp,tsId(T{iPr}(j)==ts & idPr==iPr)=j;end;end
    
    %% for one session
else % if numel(sessId)>1
    if idPCpoolSess
        if idIncEarly
...
%     % partial correlation: ts sId idCond (pr EH targ)
%     [pc,ppc]=partialcorr(sSet(:),v(:),[sId2(:) idCond(:)]); % nominal sess Id, condition inlcluding ts
%     disp(['partial correlation(Xu,Speed): ' num2str(pc) ', ' num2str(ppc)]);
%     [pc,ppc]=partialcorr(tp(:),v(:),[sId2(:) idCond(:)]);
%     disp(['partial correlation(Speed,tp): ' num2str(pc) ', ' num2str(ppc)]);
%     
%     [pc,ppc]=partialcorr(sSet(:),tp(:),[sId2(:) idCond(:)]);
%     disp(['partial correlation(sSet,tp): ' num2str(pc) ', ' num2str(ppc)]);
    ...
    %% plot mean var Xu
    figure; ha; setFigPos(1,3); box off;
    ...
    figure; ha; setFigPos(1,4); box off;
    for iPr=1:nPr
        if iPr==nPr
    ...
    xlabel('t_s (ms)'); ylabel('var. across trials');  axis tight; ylim([0.5 1.25]);
    applytofig4keynote; % applytofig4paper; %
    
        % cond-spec correlation
    cCond1=nan(length(IS),length(IC));
    cCond2=nan(length(IS),length(IC));
    
        % condition-specific z-scoring (including ts)
    % + session-specific z scoring
    for iS=1:length(IS)
        for iC=1:length(IC)
            tmpId=IC(iC)==idCond & IS(iS)==sId;
            
            sSet(tmpId)=zscore(sSet(tmpId));
            tp(tmpId)=zscore(tp(tmpId));
            v(tmpId)=zscore(v(tmpId));
            
            cCond1(iS,iC)=corr(sSet(tmpId),v(tmpId));
            cCond2(iS,iC)=corr(v(tmpId),tp(tmpId));
            
        end
    end
    
    nH=20;
    figure; ha; setFigPos(2,1); histogram(cCond1(:),nH,'facecolor','w','edgecolor','k'); plot(mean(cCond1(:)),max(ylim),'kv'); plotVertical(gca,0,[]);applytofig4keynote;
    figure; ha; setFigPos(2,2); histogram(cCond2(:),nH,'facecolor','w','edgecolor','k'); plot(mean(cCond1(:)),max(ylim),'kv'); plotVertical(gca,0,[]);applytofig4keynote;
    disp(IS);
    figure; ha; setFigPos(2,3); imagesc(cCond1); colorbar; set(gca,'ytick',1:length(IS),'yticklabel',IS);axis tight; applytofig4keynote;
    figure; ha; setFigPos(2,4); imagesc(cCond2); colorbar; set(gca,'ytick',1:length(IS),'yticklabel',IS);axis tight; applytofig4keynote;
    
    %%
    
    % remove outlier
    nZ=2; % .5;
    idValidTpSsetV=abs(sSet)<nZ & abs(tp)<nZ & abs(v)<nZ; % 27964>26965: 3.57% 
    sSet=sSet(idValidTpSsetV);
    tp=tp(idValidTpSsetV);
    ts=ts(idValidTpSsetV);
    v=v(idValidTpSsetV);
    idCond=idCond(idValidTpSsetV);
...
%     % bootstrap
nBoot=1000;
    cBoot=bootstrp(nBoot,@corr,sSet(:),v(:));
    scBoot=sort(cBoot);
    disp([scBoot(25) scBoot(975)]);
    varargout{1}=std(cBoot);
    figure; setFigPos(2,1);histogram(cBoot,35,'DisplayStyle','stairs');plotVertical(gca,0,[]);
    plot(r1,0,'ko','markersize',10,'markerfacecolor','w');
    xlabel('corr b/t Xu & speed'); ylabel('# bootstrapped corr. coef.');applytofig4keynote;

    cBoot=bootstrp(nBoot,@corr,v(:),tp(:));
    varargout{1}=std(cBoot);
        scBoot=sort(cBoot);
    disp([scBoot(25) scBoot(975)]);
    figure; setFigPos(2,2);histogram(cBoot,35,'DisplayStyle','stairs');plotVertical(gca,0,[]);
    plot(r2,0,'ko','markersize',10,'markerfacecolor','w');
    xlabel('corr b/t speed & tp'); ylabel('# bootstrapped corr. coef.');applytofig4keynote;
    
    % shuffling trial index
    nBoot=1000;
    rShf1=nan(nBoot,1);
    rShf2=nan(nBoot,1);
    for iBoot=1:nBoot
        [rShf1(iBoot),~]=corr(sSet(randi(length(sSet),length(sSet),1)),v(randi(length(v),length(v),1)));
        [rShf2(iBoot),~]=corr(v(randi(length(v),length(v),1)),tp(randi(length(tp),length(tp),1)));
    end
    figure; setFigPos(2,1);histogram(rShf1,35,'DisplayStyle','stairs');plotVertical(gca,0,[]);
    plot(r1,0,'ko','markersize',10,'markerfacecolor','w');
    xlabel('corr b/t Xu & speed'); ylabel('# shuffled corr. coef.');applytofig4keynote;
    figure; setFigPos(2,2);histogram(rShf2,35,'DisplayStyle','stairs');plotVertical(gca,0,[]);
    plot(r2,0,'ko','markersize',10,'markerfacecolor','w');
    xlabel('corr b/t speed & tp'); ylabel('# shuffled corr. coef.');applytofig4keynote;
    scBoot=sort(rShf1);
    disp([scBoot(25) scBoot(975)]);cc
...
hSurf=surf(tmpX,tmpY,N','linewidth',lw,'EdgeAlpha',0); %tmpY,tmpX,N,'linewidth',lw,'EdgeAlpha',0); % ,'ShowText','on')
% set(hSurf,'ZData',get(hSurf,'ZData')-max(get(hSurf,'ZData')));
axis tight; drawnow;
...
xlabel('z-scored projection (zXu)'); ylabel('z-scored speed (zV)'); 
axis([-nZ nZ -nZ nZ]); % axis tight; 
plotVertical(gca,0,[]); plotHorizon(gca,0,[]);
set(gca,'xtick',-4:2:4,'ytick',-4:2:4,'tickdir','out'); %%%%%
% applytofig(gcf,optsExpFig);
applytofig4keynote;

hFig2=figure;ha; setFigPos(1,2);
disp(['Speed vs tp, pearsonC: ' num2str(r2,3) ', p:' num2str(p2,3)]);
[N,xedges,yedges]=histcounts2(v(:),tp(:),nBin);
colormap(cmap);
tmpX=xedges(1:(end-1)); %+diff(xedges)/2;
tmpY=yedges(1:(end-1)); % +diff(yedges)/2;
hSurf=surf(tmpX,tmpY,N','linewidth',lw,'EdgeAlpha',0); % surf(tmpY,tmpX,N,'linewidth',lw,'EdgeAlpha',0); % ,'ShowText','on')
% set(hSurf,'ZData',get(hSurf,'ZData')-max(get(hSurf,'ZData')));
axis tight; drawnow;
...
hBar2=colorbar; hBar2.Label.String='# Trials';
set(hBar2,'Ticks',round(linspace(min(N(:)),max(N(:)),5)*100)/100); % ,'TickLabels',round(linspace(min(rMat),max(rMat),5)*100)/100);
xlabel('z-scored speed (zV)'); ylabel('z-scored tp'); 
axis([-nZ nZ -nZ nZ]); % axis tight; 
plotVertical(gca,0,[]); plotHorizon(gca,0,[]);
set(gca,'xtick',-4:2:4,'ytick',-4:2:4,'tickdir','out'); %%%%%
% applytofig(gcf,optsExpFig);
