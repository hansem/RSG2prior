function fig3c(varargin)

% 2019/4/12
% bootstrapping
% check RMSE by different wm

% 2019/4/11
% adding mean tp
% model-free bias: deltaTp (8 data sets)

% 2019/3/22
% measureing slope: idSiL=0
...
% G handRight as representative
% save xu in trajKS_prior*.mat
...
idSiL=0; % 0; % for speed, 1; % 0; % also analyzing SiL data
...
% model-free; now mean tp 2019/4/11
dXu=nan(nDS,nPr,2); % % last for middle vs extreme
...
for iDS=1:nDS
    for iPr=1:nPr
       ...
       muXu=xu2(1:nBin:end);
       dXu(iDS,iPr,1)=mean(diff(muXu(2:4)));
       dXu(iDS,iPr,2)=mean([diff(muXu(1:2)) diff(muXu(4:5))]);
        
       dTp(iDS,iPr,1)=mean(diff(mut(iDS,iPr,2:4)));
       dTp(iDS,iPr,2)=mean([diff(mut(iDS,iPr,1:2)) diff(mut(iDS,iPr,4:5))]);
    end % for iPr=1:nPr
end % for iDS=1:nDS
% plot
figure; ha; setFigPos(2,1);
    tmpX=dXu(:,:,1); tmpX=tmpX(:);
    tmpY=dXu(:,:,2); tmpY=tmpY(:);
%     plot(tmpX,tmpY,'ko');
for iPr=1:nPr
    plot(dXu(:,iPr,1),dXu(:,iPr,2),'o','color',cmapPr(iPr,:));
end
axisEqual % axis tight; 
plotIdentity(gca);
ylabel('\DeltaXu for extreme t_s');
xlabel('\DeltaXu for middle t_s');
applytofig4keynote
% histogram
figure; ha; setFigPos(2,2);
nBin=12;
ddXu=squeeze(-dXu(:,:,1)+dXu(:,:,2)); % extreme - middle (<0 expected)
x=linspace(min(ddXu(:)),max(ddXu(:)),nBin);
% hHist=histogram(tmpY-tmpX,25);
% set(hHist,'Facecolor','w');
for iPr=1:nPr
    h=histogram(squeeze(ddXu(:,iPr)),x);
    
    if idSiL
        Xval    = h.BinEdges + h.BinWidth*0.5 +h.BinWidth*(iPr-2)*0.3; % (i-(nPr+1)/2)/2; % binWidth*0.25 for short, 0.75 for long > binWidth*0.2 for short, 0.5 for long, 0.8 for SiL
    else
        Xval    = h.BinEdges + h.BinWidth*0.5 +h.BinWidth*(i-(nPr+1)/2)/2; % binWidth*0.25 for short, 0.75 for long > binWidth*0.2 for short, 0.5 for long, 0.8 for SiL
    end
     Xval    = Xval(1:end-1);
     Yval    = h.Values;
     delete(h);
...
axis tight;
maxYlim=max(ylim);
for iPr=1:nPr % plot mean
    plot(mean(ddXu(:,iPr)),1.1*maxYlim,'color',cmapPr(iPr,:),'markerfacecolor',cmapPr(iPr,:),'marker','v','markersize',markersize);
end % for i=1:nPr
xlabel('\DeltaXu (extreme)-\DeltaXu (middle)'); ylabel('# data sets'); set(gca,'tickdir','out','ticklength',[0.02 0.02],'xtick',-0.4:0.2:0.4,'ytick',0:2:10); % 'xtick','','ytick',''
plotVertical(gca,0,[]);
ylabel('# data sets');
applytofig4keynote
% plot dTp vs dXu
hFig=figure; ha; setFigPos(2,3);
for iPr=1:(nPr-1)
    plot(ddXu(:,iPr),squeeze(dTp(:,iPr,2)-dTp(:,iPr,1)),'o','color',cmapPr(iPr,:));
%     plotReg(ddXu(:,iPr),squeeze(dTp(:,iPr,2)-dTp(:,iPr,1)),hFig,cmapPr(iPr,:),0,1);
end
axis([ -0.4   0.1 -37.5429 3]); % -0.0316  -37.5429  -10.9408]); %  tight; 
% plotIdentity(gca);
plotVertical(gca,0,[]);
plotHorizon(gca,0,[]);
set(gca,'xtick',[-0.4:0.2:0],'ytick',[-30:15:0],'tickdir','out');
ylabel('\Deltat_p (extreme)-\Deltat_p (middle)'); 
xlabel('\DeltaXu (extreme)-\DeltaXu (middle)'); 
applytofig4keynote
% plot ratio (dTp vs dXu)
hFig=figure; ha; setFigPos(2,4);
for iPr=1:(nPr-1)
    plot(abs(dXu(:,iPr,2)./dXu(:,iPr,1)),squeeze(dTp(:,iPr,2)./dTp(:,iPr,1)),'o','color',cmapPr(iPr,:));
%     plotReg(abs(dXu(:,:,2)./dXu(:,:,1)),squeeze(dTp(:,iPr,2)./dTp(:,iPr,1)),hFig,cmapPr(iPr,:),0,1);
end
axis([0.4000    0.8824    0.6000    0.9000]);
ylabel('\Deltat_p (extreme)/\Deltat_p (middle)'); 
xlabel('\DeltaXu (extreme)/\DeltaXu (middle)'); 
applytofig4keynote
...
%% inset for fig
figure; setFigPos(2,3); ha; % plot session-specific but avgAcrossConditions
msize=5;
    sS=squeeze(sl(:,1));
    sL=squeeze(sl(:,2));
...
% a=[1:5]+rand(1,5);
% a=a-3;
% regress(a',[T{1}(:) ones(5,1)])
%     0.0136
%    -8.2115
% 
% regress(a',[T{2}(:) ones(5,1)])
%     0.0109
%   -10.3905
%   
%   minXu=cellfun(@min,xu)
% maxXu=cellfun(@max,xu)
% tmp=maxXu-minXu;
% figure; plot(tmp(:,1),tmp(:,2),'o')
% plotIdentity(gca)
% axis tight
% xlabel('max(Xu)-min(Xu) (short)');
% ylabel('max(Xu)-min(Xu) (long)');

%% figure for response R3
% slope for EL vs HL; ER vs EL
figure; setFigPos(2,4); ha;
msize=5;

plot([1:2]',sl([2 6],1),'-','color',[.5 .5 .5],'linewidth',.5); % connecting lines H:EL-HL
plot([1:2]',sl([4 8],1),'-','color',[.5 .5 .5],'linewidth',.5); % connecting lines H:ER-HR
errorbar(1,sl(2,1),slVar(2,1),'marker','d','color',pplot.cmap{4},'markersize',msize,'markerfacecolor','w');
errorbar(2,sl(6,1),slVar(6,1),'marker','^','color',pplot.cmap{1},'markersize',msize,'markerfacecolor','w');
errorbar(1,sl(4,1),slVar(4,1),'marker','s','color',pplot.cmap{2},'markersize',msize,'markerfacecolor','w');
errorbar(2,sl(8,1),slVar(8,1),'marker','o','color',pplot.cmap{3},'markersize',msize,'markerfacecolor','w');
set(gca,'xtick',1:2,'xticklabel',{[];[]},'xlim',[0.9 2.1],'tickdir','out','ticklength',[0.03 0.03]); % ,'ytick',0:.5:1
