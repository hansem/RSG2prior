function runPSTH2traj_ts_tp_conditionSpecific

% 2019/1/20
% re-run after manual update of behavioral idOut
% remove 'bin20_smth40' at file name end
...
%     for iHE=1:nEH
%         for iTarg=1:nTarg
%             [D,proj_matrix,keep_neurons,eigenvalues,meanPSTH]=PSTH2traj(D(((iHE-1)*nEH+iTarg):nCond:length(D)),...
%                 [],[],binSize,smthWidth,[],use_sqrt); %  proj_matrix,keep_neurons optimD
%             meanPSTH=meanPSTH(:);
%             fnTmp=['trajKS_ts_' ehNm{iHE}(1) targNm{iTarg}(1) '_' animalNm{iAnimal} '.mat']; %  '_bin' num2str(binSize) '_smth' num2str(smthWidth)
%             optimD=size(D(1).data,1);
%             if idSave % ~exist(fnTmp,'file')
%                 save(fnTmp,...
%                     'D','proj_matrix','keep_neurons','binSize','smthWidth','optimD','use_sqrt','eigenvalues','meanPSTH');
%             end
%             D=oldD;
%             
%             if idPlotTraj
    ...
    for iHE=1:nEH
        for iTarg=1:nTarg
            [D,proj_matrix,keep_neurons,eigenvalues,meanPSTH]=PSTH2traj(D(((iHE-1)*nEH+iTarg):nCond:length(D)),...
                [],[],binSize,smthWidth,[],use_sqrt); %  proj_matrix,keep_neurons optimD
            meanPSTH=meanPSTH(:);
            fnTmp=['trajKS_tp_' ehNm{iHE}(1) targNm{iTarg}(1) '_' animalNm{iAnimal} '.mat']; % '_bin' num2str(binSize) '_smth' num2str(smthWidth)
            optimD=size(D(1).data,1);
            if idSave %~exist(fnTmp,'file')
                save(fnTmp,...
                    'D','proj_matrix','keep_neurons','binSize','smthWidth','optimD','use_sqrt','eigenvalues','meanPSTH');
