...
if strcmp(get(a,'visible'),'on')
  ...
  if iscell(ylabs), ylabs=char(ylabs); end
  yticks = get(a,'ytick');
  maxw = 0;
  if ~isempty(ylabs)
    ...
    ry(4) = ry(4) + ext(4);
    if ~isempty(yticks)
        if ~isempty(str2num(ylabs(1,:))) & ~isempty(str2num(ylabs(end,:)))
      if ~strcmp(get(a,'yscale'),'log') & ...
  ...
  if iscell(xlabs), xlabs=char(xlabs); end
  xticks = get(a,'xtick');
  if ~isempty(xlabs)
    ...
    if ~isempty(xticks)
        if ~isempty(str2num(xlabs(1,:))) & ~isempty(str2num(xlabs(end,:)))
      if ~strcmp(get(a,'xscale'),'log') & ...
