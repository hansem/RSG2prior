...
%
%% modified for 3D (hansem@mit.edu June 24 2017)
%
%  r=sort(rand(10,1))*2*pi;
%  Vertices=[sin(r) cos(r) ones(size(r))]*10; % for 3D
%  Lines=[(1:size(Vertices,1))' (2:size(Vertices,1)+1)']; Lines(end,2)=1;
%  k=LineCurvature2D(Vertices,Lines);
% 
%  figure,  hold on;
...
% for 2D, plus for convex, minus for concave
% 2018/1/16: k=abs(k);
...
if size(Vertices,2)==3 % 3D
    % z=c(3)*t^2 + c(2)*t + c(1) 
    z = [Vertices(Na,3) Vertices(:,3) Vertices(Nb,3)];
    c(:,1)=invM(:,1,1).*z(:,1)+invM(:,2,1).*z(:,2)+invM(:,3,1).*z(:,3);
    c(:,2)=invM(:,1,2).*z(:,1)+invM(:,2,2).*z(:,2)+invM(:,3,2).*z(:,3);
    c(:,3)=invM(:,1,3).*z(:,1)+invM(:,2,3).*z(:,2)+invM(:,3,3).*z(:,3);
    
    dr=[a(:,2)'; b(:,2)'; c(:,2)']; % [3D x time]
    dr2=[a(:,3)'; b(:,3)'; c(:,3)']; % [3D x time]
    
    k=normCol(cross(dr,dr2,1))./(normCol(dr).^3); % [1 x time]
...
function y=normCol(x,varargin)

% function y=normCol(x,varargin)
% varargin for dimesion to calculate norm
% default is calculating norm columnwise
