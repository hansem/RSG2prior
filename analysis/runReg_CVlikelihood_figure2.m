function runReg_CVlikelihood_figure2

%% plot CVlikelihood(Short|Short) vs CVlikelihood(Short in Long|Short)

% 1) split short prior data into half and fit GLM
% 2) calculate likelihood of the remaining short-prior data & shortInLong
...
% H, 17 sessions, 34 proble penetrations, 196 single 421 multi > 617 total > 538 in PSTH mat (nTrialExc=5) > 519 in reg.mat (thNT:5, thFR:1)
% G, 12 sessions, 35 proble penetrations, 260 single 481 multi > 741 total > 717 in PSTH mat (nTrialExc=5) > 637 in reg.mat (thNT:5, thFR:1)
...
load pplot.mat;
if ~exist(fullfile(psthDir,'regCV.mat')) % exist('reg.mat')
...
d=[]; % deviance: -2*(LL(model)-LL(saturated model)): [full noTs noTp noPrior wInteract noEH noTarg]
...
%                 if  iCn % only for intersting neurons
                    ...
                    statTmp=regCV(NEV,tmpId,tInfo(~idOut2,:),eventId,idPlot,regP); % {[2(b,p) x nReg]; meanFR[10 x1]}
                    if ~isempty(statTmp)
                         b=[b; statTmp.b(:)']; % const, ts, ts2, prior tsXpr ts2Xpr
                        p=[p; statTmp.p(:)'];
                        ...
                        mLLS=[mLLS; statTmp.mLLS];
                        mLLSL=[mLLSL; statTmp.mLLSL];
...
%                     if idPlot
%                     set(gcf,'position',pplot.(['rect' num2str(ceil(iPlot./6)) '_' num2str(rem(iPlot-1,6)+1)]));
%                     title([fid '        ' num2str(idSU(j,1)) ' ' type{cn(iCn,3)+1}]);
%                     drawnow;
% %                     axis tight; 
%                     xlabel('# spikes (80ms bin before set)'); ylabel('t_p');
%                     if iPlot==12
%                         iPlot=1; waitforbuttonpress; close all;
%                     else
%                         iPlot=iPlot+1;
%                     end
%                     end % if idPlot
                    
%                 end % if str2num(fid)==cn(:,1) & idSU(j,1)==cn(:,1) % only for intersting neurons
...
save('regCV.mat'); % b p c 

else
    load('regCV.mat');
    reg=load('reg.mat'); % to find ts-mod cells
end
...
disp(['H # neurons: ' num2str(nnz(reg.c(:,1)<170000))]);
disp(['G # neurons: ' num2str(nnz(reg.c(:,1)>170000))]);
...
modelN={'full'; 'noTs'; 'noTp'; 'noPrior'; 'wInteract'; 'noEH'; 'noTarg'};
...
dBICts=reg.d(:,1)-reg.d(:,2)+(reg.nP(:,1).*log(reg.nT)-reg.nP(:,2).*log(reg.nT)); % -2*LLR(full/ts-reduced)
dBICtp=reg.d(:,1)-reg.d(:,3)+(reg.nP(:,1).*log(reg.nT)-reg.nP(:,3).*log(reg.nT)); % -2*LLR(full/tp-reduced)
dBIC=bsxfun(@minus,d(:,1),d(:,2:end))+... % repmat(d(:,1),1,nModel-1)-d(:,2:end)+...
    bsxfun(@minus,reg.nP(:,1).*log(reg.nT),bsxfun(@times,reg.nP(:,2:end),log(reg.nT))); %     (repmat(nP(:,1),1,nModel-1).*log(nT)-nP(:,2:end).*log(nT)); % -2*LLR(full/tp-reduced)

idTsMod=dBICts<0; disp(['% ts-modulation: ' num2str(mean(idTsMod)*100)]);
idPrMod=dBIC(:,3)<0; disp(['% prior-modulation: ' num2str(mean(idPrMod)*100)]);
...
% test independence b/t ts-modulated and prior-modulated neurons
 ...
 for i=1:nAnimal
     nMatAn{i}=[nnz(~idTsMod&~idPrMod&idA==i) nnz(~idTsMod&idPrMod&idA==i);...
         nnz(idTsMod&~idPrMod&idA==i) nnz(idTsMod&idPrMod&idA==i)];     
     disp(nMatAn{i});
     disp(nMatAn{i}./sum(nMatAn{i}(:))*100);
...
%% plot CVlikelihood(Short|Short) vs CVlikelihood(Short in Long|Short)
% plot all cells
% (no) idTsMod&:only plot cell modulated by ts
...
minTmp=0;
maxTmp=0.81;maxTmp2=0.8;
xlim([minTmp maxTmp]);
ylim([minTmp maxTmp]);
% set(gca,'xscale','log','yscale','log')
plotIdentity(gca);

plot(exp(mLLS(1==idA)),exp(mLLSL(1==idA)),'.','markerfacecolor','w','markersize',msize,'linewidth',lw,'color','r');ha;
plot(exp(mLLS(nAnimal==idA)),exp(mLLSL(nAnimal==idA)),'.','markerfacecolor','w','markersize',msize,'linewidth',lw,'color','b');ha;
% plot(mLLS(idTsMod&1==idA),mLLSL(idTsMod&1==idA),'o','markerfacecolor','w','markersize',msize,'linewidth',lw,'color','r');ha;
% plot(mLLS(idTsMod&nAnimal==idA),mLLSL(idTsMod&nAnimal==idA),'^','markerfacecolor','w','markersize',msize,'linewidth',lw,'color','b');ha;

set(gca,'tickdir','out','TickLength', [.02 .02],'xtick',minTmp:0.2:maxTmp2,... %'xticklabel',,...
            'ytick',minTmp:0.2:maxTmp2); % ,... 'yticklabel',);

% xlabel('log p(short|GLM fit to short)');
% ylabel('log p(short in long|GLM fit to short)');

% outliers: SL is so different from S
% c(mLLSL<-3.5&idTsMod,:)
% 
%       161221        1133
%       170821        2142: too low FR
%       170823        1166

idOut=mLLSL<-3.5;

figure; setFigPos(1,2);
tmp=mLLS(1==idA & ~idOut)-mLLSL(1==idA & ~idOut);
histogram(tmp,'DisplayStyle','stairs','linewidth',lw,'Normalization','count','BinMethod','auto','EdgeColor','r');ha;
errorbar(mean(tmp),75,sem(tmp),'horizontal','linewidth',2,'color','r');
tmp=mLLS(nAnimal==idA & ~idOut)-mLLSL(nAnimal==idA & ~idOut);
histogram(tmp,'DisplayStyle','stairs','linewidth',lw,'Normalization','count','BinMethod','auto','EdgeColor','b');ha;
errorbar(mean(tmp),55,sem(tmp),'horizontal','linewidth',2,'color','b');
xlabel('log p(short)-log p(short in long)'); ylabel('# cells'); plotVertical(gca,0,[]);

pH=signrank(mLLS(1==idA & ~idOut),mLLSL(1==idA & ~idOut));
disp(['H: p(signrank)=' num2str(pH)]);
pG=signrank(mLLS(nAnimal==idA & ~idOut),mLLSL(nAnimal==idA & ~idOut));
disp(['G: p(signrank)=' num2str(pG)]);
...
% DS=fillNan(D(1:(nTspp*nEH*nTarg)));
% DL=fillNan(D((nTspp*nEH*nTarg+1):end));
