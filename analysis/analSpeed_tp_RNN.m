...
% chagne in kinet
% tInitCond: 100 (<200 in data)
% measuring speed from 0
% use robustfit for slope estimation
...
maxNbin=64; % RNN 44; % tp-specific

% speed & arc length (whole meas/overlap)
v=nan(nRNN,nEH,nTarg,nPr,nTs,maxNbin); % speed [2HG x 2EH x 2RL x 2SL x 5ts x 60timePoints]
vv=nan(nRNN,nEH,nTarg,nPr,nTs,maxNbin,maxDim); % velocity vector vv(idAnimal,iEH,iTh,idPr,iTs,1:nV,1:dim)[2HG x 2EH x 2RL x 2SL x 5ts x 60timePoints x dim]
v2=nan(nRNN,nEH,nTarg,nPr,nTs,maxNbin); % ||2nd derivative|| [2HG x 2EH x 2RL x 2SL x 5ts x 60timePoints]
c=nan(nRNN,nEH,nTarg,nPr,nTs,maxNbin); % curvature [2HG x 2EH x 2RL x 2SL x 5ts x 60timePoints]
nEpoch=1; % 3; % wholeMeas/overlap, short/long, SIL
x=nan(nRNN,nEH,nTarg,nPr,nTs,nEpoch); % arc length [2HG x 2EH x 2RL x 2SL x 5ts x 60timePoints]
...
% speed & distance
sp=nan(nRNN,nEH,nTarg,nPr,nTs); % [2HG x 2EH x 2RL x 2SL x 5ts]
dist=nan(nRNN,nEH,nTarg,nPr,nTs); % [2HG x 2EH x 2RL x 2SL x 5ts]
...
sSp=nan(nRNN,nEH,nTarg,nPr,nTs,maxNbin,maxDim); % [2HG x 2EH x 2RL x 2SL x 5ts x timePoints]
tSp=nan(nRNN,nEH,nTarg,nPr,nTs,maxNbin); % [2HG x 2EH x 2RL x 2SL x 5ts x timePoints] in refD's bin unit
spv=nan(nRNN,nEH,nTarg,nPr,nTs,maxNbin,maxDim); % [2HG x 2EH x 2RL x 2SL x 5ts x timePoints x dim]

% 1) angle b/t velocity vectors of short vs long as a function of time: if <90, parallel
av=nan(nRNN,nEH,nTarg,nPr,nTs,maxNbin,nPr,nTs); % [2HG x 2EH x 2RL x 2SL x 5ts x timePoints x (2SL x 5ts)]
av2=cell(nRNN,nEH,nTarg,nPr,maxNbin); % [2HG x 2EH x 2RL x within/betweenPriors x timePoints]
% 2) shortest path length between short and long: constant?
spl=nan(nRNN,nEH,nTarg,nPr,nTs,maxNbin); % [2HG x 2EH x 2RL x 2SL x 5ts x timePoints]
% 3) angle b/t tangent and shortest path vector: if <90, on the same manifold
aSpvTv=nan(nRNN,nEH,nTarg,nPr,nTs,2,maxNbin); % [2HG x 2EH x 2RL x 2SL x 5ts x 2tv(ts,ref) x timePoints]


%% tp-specific
tInitCond=100; % 200;%400; % 200;
initT=tInitCond/binSize; % 10th bin
...
aTsvSv2=nan(nRNN,nEH,nTarg,nPr,nTs); % prod-space % [2HG x 2EH x 2RL x 2SL x 5ts x timePoints]; for short800, tsv to long800, for 1200, tsv from 1100
%    - assuming curved trajectory is in 2D (check: scree?), projecting time vector to the 2D plane>angle b/t the projected time vector and rotating vector
aTsvSv1=nan(nRNN,nEH,nTarg,nPr,nTs); % meas-space
%    - do PCA separately for set and IC: angle between proj_matrix (or CCA?)
%    - regression? how to choose set-responsive neuron?
% 2) check angle/magnitude b/t readout vectors across ts, prior: 2D cmap, if <90, parellel-not state dependent
%    - kinet: nearest distance coalesce during set transient?
asv=nan(nRNN,nEH,nTarg,nPr*nTs,nPr*nTs);
asv2=cell(nRNN,nEH,nTarg,nPr); % within/betweenPriors
msv=nan(nRNN,nEH,nTarg,nPr,nTs);
% 3) curvature b/t Set vs IC (200ms): scatter, separately for short and long
%   - 2D @ set, 1D @ set
% only in meas-subspace as rotation is not clear in prod-subspace
cAcrTs=nan(nRNN,nEH,nTarg,nPr,2,nTspp-2); % [2HG x 2EH x 2RL x 2SL x @set/@IC x 3data points]

% 1) IC vector size (ref to each prior mean) or their relative angle (more curved?): without finding out nearest neighbors
tsv=nan(nRNN,nEH,nTarg,nPr*nTs-1,maxDim,maxNbin); % [2HG x 2EH x 2RL x 9 diffTs x time x dim]: for short800, tsv to long800
mtsv=nan(nRNN,nEH,nTarg,nPr*nTs-2,maxNbin); % magnitude of tsv [2HG x 2EH x 2RL x 9 diffTs x time]: for short800, tsv to long800
atsv=nan(nRNN,nEH,nTarg,nPr*nTs-1,maxNbin); % angle b/t nearby tsv [2HG x 2EH x 2RL x 9 diffTs x time]: for short800, tsv to long800
% 2) angle b/t overlap vector & time/ts vector: common time-warped axis?
aTsvOv=nan(nRNN,nEH,nTarg,nPr*nTs-2,maxNbin);
aVvOv=nan(nRNN,nEH,nTarg,nPr*nTs,maxNbin);% vv=nan(nRNN,nEH,nTarg,nPr,nTs,maxNbin); % velocity vector [2HG x 2EH x 2RL x 2SL x 5ts x 60timePoints x dim]
% 3) angle/magnitude of time vectors (ref: prior mean)
% use asv, msv
% 4) angle b/t common IC axis
aCommIC=nan(nRNN,nEH,nTarg,maxNbin);
...
%% cosyne
mtp=nan(nRNN,nEH,nTarg,nPr,nTs); % speed [2HG x 2EH x 2RL x 2SL x 5ts]
sICa=nan(nRNN,nEH,nTarg,nPr,nTs); % projection of states onto IC axis

% d=dir('trajKS_tp_*bin20_smth40.mat');



for idAnimal=1:nRNN % iFile=1:length(d)
...
%     if ~isempty(strfind(d(idAnimal).name,'_H_'))
%         idAnimal=1; iAnimalNm='H';
%     else
%         idAnimal=2; iAnimalNm='G';
%     end
    if ~isempty(strfind(d(idAnimal).name,'_ER_')) || ~isempty(strfind(d(idAnimal).name,'_Weber_')) % RNN
        iEH=1; iTarg=1;
    elseif ~isempty(strfind(d(idAnimal).name,'_EL_'))
        iEH=1; iTarg=2;
    elseif ~isempty(strfind(d(idAnimal).name,'_HR_'))
        iEH=2; iTarg=1;
    elseif ~isempty(strfind(d(idAnimal).name,'_HL_'))
        iEH=2; iTarg=2;
    end
    tmpId=[iEH; iTarg];
    ...
    if idProd % idMeas
        ...
        for i=1:length(tmpD) % 10 ts
            ...
            tmpTsp=(tmpTsp(initTtmp:end)-initTtmp)*binSize; % speed after IC
%             sp(idAnimal,iEH,iTh,idPr,iTs)=squeeze(tmpTsp)\(binSize*[0:(length(tmpTsp)-1)]');
xTmp=(binSize*[0:(length(tmpTsp)-1)]');
yTmp=squeeze(tmpTsp);
B=robustfit(xTmp,yTmp);
sp(idAnimal,iEH,iTh,idPr,iTs)=B(2);
        ...
        for iPr=1:nPr
            sICa(idAnimal,iEH,iTh,iPr,:)=sSetTmp((iPr-1)*nTspp+[1:nTspp],:)*axIC;
...
%         for iTarg=1:nTarg
            for iPr=1:nPr
                for iTs=1:nTspp
                    idTp=idShortTrial==(2-iPr) & ts==T{iPr}(iTs);
                    mtp(idAnimal,iEH,iTarg,iPr,iTs)=mean(tp(idTp));
...
figure(1); ha;setFigPos(1,4); % 1); % speed vs time (avg. across ts) two lines for short vs long
figure(2); ha;setFigPos(1,5); % 2); % avv
% figure(3); setFigPos(1,3); % avv colormap for each data set: short
% figure(4); setFigPos(2,3); % smae for long

condNm={'ER','EL','HR','HL'};
nCmap=nRNN; % length(animalNm)*length(condNm);
cmap3=cell2mat(pplot.cmap); % parula(nCmap); % hsv(nCmap);
icmap=1;
...
for i=1:nRNN
    for j=1:nEH
        for k=1:nTarg
            ...
            plot(tmpX(1:nTs),tmpY(1:nTs),'o-','color',cmap3(i,:),'linewidth',lw2,'markerfacecolor','w'); %short cmapPr2(1,:)
            plot(tmpX(nTs+[1:nTs]),tmpY(nTs+[1:nTs]),'x-','color',cmap3(i,:),'linewidth',lw2,'markerfacecolor','w'); %long cmapPr2(2,:)
            X=[X; tmpX(:)']; % [8dataSet x 10ts]
            Y=[Y; tmpY(:)']; % [8dataSet x 10ts]
            if icmap==nCmap
                % avg across data sets: X for RNN
...
%                         tmpCmap{idPr,1}(iTs,:),'o',msize,tmpCmap{idPr,1}(iTs,:),'k');
%                 end
                axis tight;
                xlabel('speed'); ylabel('t_p');
%                 set(gca,'tickdir','out','xtick',0.1:0.2:0.3,'xlim',[0.08 0.4],... % ,'xlim',[T{1}(1)-binSize T{end}(end)+binSize],'xticklabel',{T{1}(1);[];T{1}(end);[];T{2}(end)}
%                     'ylim',[450 1250],'ytick',[T{1}(1);T{1}(end);T{2}(end)],'yticklabel',[T{1}(1);T{1}(end);T{2}(end)]./1000);
            end %  if icmap==nCmap
...
%                 end
                axis tight;
                xlabel('S(IC)'); ylabel('speed');
%                 set(gca,'tickdir','out','ytick',0.1:0.2:0.3,'ylim',[0.08 0.4],... %); % ,'xtick',[T{1}(1:2:end) T{2}(3:2:end)],'xlim',[T{1}(1)-binSize T{end}(end)+binSize],'xticklabel',{T{1}(1);[];T{1}(end);[];T{2}(end)},...
%                     'xtick',-2:1);
...
teSp=nan(nRNN*nEH*nTarg,nPr,nTs); % 1/speed fit to BLS to avg
figure(1001); setFigPos(2,5); % distance = f(ts) with BLS
teDist=nan(nRNN*nEH*nTarg,nPr,nTs); % dist fit to BLS

condNm={'ER','EL','HR','HL'};
nCmap=nRNN; %nCmap=length(animalNm)*length(condNm);
cmap4=cell2mat(pplot.cmap); % parula(nCmap); % hsv(nCmap);
icmap=1;
...
%                 for iTs=1:nTs
                    if idTpMTs
                        plot(T{iPr}(:),stats.yhat(:)-T{iPr}(:),'-','color',cmap4(icmap,:),'markerfacecolor','w','linewidth',lw2,'markersize',msize); %cmap4(icmap,:),'markerfacecolor','w');
                                                c=corr(stats.yhat(:)-T{iPr}(:),BLS(T{iPr}(:),0.05,[min(T{iPr}(:)) max(T{iPr}(:))],'uniform')-T{iPr}(:));
                        disp(['R^2(BLS-ts): ' num2str(c.^2,2)]);
                    else
                        plot(T{iPr}(:),stats.yhat(:),'-','color',cmap4(icmap,:),'markerfacecolor','w','linewidth',lw2,'markersize',msize); %cmap4(icmap,:),'markerfacecolor','w');
                                                c=corr(stats.yhat(:),BLS(T{iPr}(:),0.05,[min(T{iPr}(:)) max(T{iPr}(:))],'uniform'));
...
%                 subplot(nRNN,nEH*nTarg,iCond*nRNN-(nRNN-i)); ha;
%                 title([animalNm{i} ':' ehNm{j} targNm{k}]);
                for l=1:nPr
                    subplot(nRNN,nPr,(i-1)*nPr+l); ha;
                    for m=1:nTspp
                ...
                axis tight;
                if i~=nRNN
                    set(gca,'xticklabel',[],'tickdir','out','ticklength',[0.02 0.02]);
                else
                    set(gca,'ticklength',[0.03 0.03]);
                    
                    subplot(nRNN,nPr,round(nRNN/2)*nPr+1); ylabel('time to nearest state (ms)'); % 5
                    subplot(nRNN,nPr,nRNN*nPr-1); xlabel('time in ref. (ms)'); % 7
                end
            end
...
%                 title([animalNm{i} ':' ehNm{j} targNm{k}]);
                ...
                axis tight;
                if i~=nRNN % if iCond~=4
                    set(gca,'xticklabel',[],'xtick',[T{1}(1:2:end) T{2}(3:2:end)],'tickdir','out','ticklength',[0.02 0.02]);
                else
                    set(gca,'xtick',[T{1}(1:2:end) T{2}(3:2:end)],'tickdir','out','ticklength',[0.02 0.02]);
                    subplot(nRNN,1,nRNN); xlabel('time after set (ms)');
                    subplot(nRNN,1,round(nRNN/2)); ylabel('distance to nearest state (ms)'); 
                end
            end
